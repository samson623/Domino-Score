...
class SupabaseMCPServer {
          ...
          },
          {
            ...
            inputSchema: {
              ...
              properties: {
                ...
                gameType: {
                  type: 'string',
                  description: 'Type of game (dominoes, custom, etc.)'
                },
                players: {
                  type: 'array',
                ...
                initialData: {
                  type: 'object',
                  description: 'Initial game data/scores'
                }
              },
          ...
          },
          {
            ...
            inputSchema: {
              ...
              properties: {
                ...
                duration: {
                  type: 'string',
                  description: 'Game duration (ISO 8601 interval)'
                }
              },
              required: ['gameId']
            }
    ...
    try {
      ...
      return {
        content: [
          {
            type: 'text',
            text: `Successfully inserted data into ${table}:\n${JSON.stringify(result, null, 2)}`
          }
        ]
      };
    } catch (error) {
    ...
    try {
      ...
      return {
        content: [
          {
            type: 'text',
            text: `Successfully updated ${result.length} records in ${table}:\n${JSON.stringify(result, null, 2)}`
          }
        ]
      };
    } catch (error) {
    ...
    try {
      ...
      return {
        content: [
          {
            type: 'text',
            text: `Successfully deleted ${result.length} records from ${table}`
          }
        ]
      };
    } catch (error) {
  ...
  async handleGetUserGames(args) {
    try {
      ...
      if (error) {
        throw new McpError(
          ErrorCode.InternalError,
          `Failed to get user games: ${error.message}`
        );
      }
      ...
      return {
        content: [
          {
            type: 'text',
            text: `User Games (${data.length} results):\n${JSON.stringify(data, null, 2)}`
          }
        ]
      };
    } catch (error) {
      throw new McpError(
        ErrorCode.InternalError,
        `Get user games failed: ${error.message}`
      );
    }
  }
  ...
  async handleGetUserBilling(args) {
    try {
      ...
      if (error && error.code !== 'PGRST116') { // PGRST116 is "not found"
        throw new McpError(
          ErrorCode.InternalError,
          `Failed to get user billing: ${error.message}`
        );
      }
      ...
      throw new McpError(
        ErrorCode.InternalError,
        `Get user billing failed: ${error.message}`
      );
    }
  }
  ...
  async handleCreateGame(args) {
    try {
      ...
      throw new McpError(
        ErrorCode.InternalError,
        `Create game failed: ${error.message}`
      );
    }
  }
  ...
  async handleUpdateGame(args) {
    try {
      ...
      throw new McpError(
        ErrorCode.InternalError,
        `Update game failed: ${error.message}`
      );
    }
  }
    ...
    try {
      ...
      return {
        content: [
          {
            type: 'text',
            text: `Leaderboard (${leaderboard?.length || 0} results):\n${JSON.stringify({ leaderboard, userStats }, null, 2)}`
          }
        ]
      };
    } catch (error) {
